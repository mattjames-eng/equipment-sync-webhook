...
if (crewMemberId && linkedProjectId) {
  try {
    ...
    const matchingAssignment = assignments.find(a => {
      const linksToProject = a.column_values.some(c => 
        c.linked_items?.some(link => link.id === String(linkedProjectId))
      );
      const matchesName = linkedProjectName && a.name.toLowerCase().includes(linkedProjectName.toLowerCase());
      return linksToProject || matchesName; 
    });
