 ...
 * - Two-step Flex API lookup (Search by Quote # -> Get internal UUID -> Get Header Data)
...
async function fetchFlexQuoteData(quoteId) {
    // STEP 1: Search Flex to find the internal UUID using the human-readable elementNumber
    const searchUrl = `${FLEX_BASE_URL}/api/element?elementNumber=${encodeURIComponent(quoteId)}`;
    ...
    const searchResults = searchData.content || searchData.elements || searchData;
