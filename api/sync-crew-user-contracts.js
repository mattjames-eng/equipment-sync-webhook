...
// Fires when Tech/Engineer relation changes on Crew Contracts board.
// Looks up the monday.com User from Crew Database and writes it to the Crew User column.
// Self-contained — no cross-function imports.

const MONDAY_API_URL = 'https://api.monday.com/v2';
const USER_COLUMN_ID = 'multiple_person_mm4m4jvs'; // "User" col in Crew Database
const TARGET_BOARD_ID = '18415879229';              // Crew Contracts
const TARGET_COLUMN_ID = 'multiple_person_mm4me99f'; // "Crew User"
...
export default async function handler(req, res) {
  ...
  // Tech cleared — wipe the people column too
  ...
  // Write the user to Crew User column
  ...
  if (updateResult?.errors) {
    console.error('[sync-crew-user-contracts] API error:', JSON.stringify(updateResult.errors));
    return res.status(500).json({ error: 'Failed to update column', details: updateResult.errors });
  }

  console.log(`[sync-crew-user-contracts] synced item=${triggeredItemId} crew_member=${crewMemberItemId}`);
